...
class MinuteHand extends StatefulWidget {
  ...
  MinuteHand({required this.minute, required this.second});

  @override
  _MinuteHandState createState() => _MinuteHandState();
}

class _MinuteHandState extends State<MinuteHand> with SingleTickerProviderStateMixin {
  ...
  _panUpdateHandler(DragUpdateDetails d) {
    ...
    setState(() {
      ...
      var degrees = roundToBase(a.roundToDouble(), 5);
      _valueChoose = degrees ~/ 6 == 60 ? 0 : degrees ~/ 6;
      BlocProvider.of<ClockBloc>(context).add(ClockEventSetMinute(_valueChoose));
    });
  }

  _panEndHandler(DragEndDetails d) {
    var a = degree < 360 ? degree.roundToDouble() : degree - 360;
    ctrl.animateTo(roundToBase(a.roundToDouble(), 10), duration: const Duration(milliseconds: 551), curve: Curves.easeOutBack).whenComplete(() {
      setState(() {
              ...
              return Transform.rotate(
                ...
                child: AspectRatio(
                    ...
                    child: Container(
                      ...
                      child: CustomPaint(painter: MinuteHandPainter(minutes: widget.minute, seconds: widget.second)),
                    )
                ),
              );
            },
